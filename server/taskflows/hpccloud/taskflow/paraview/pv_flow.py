...
# -----------------------------------------------------------------------------
# Local files that need to be deployed
# -----------------------------------------------------------------------------

LOCAL_DIRECTORY = '/hpccloud/pvw-dependencies/pv-flow'

LOCAL_FILES = [
    'pvw-flow.py',
    'flow/processing/__init__.py',
    'flow/processing/engine.py',
    'flow/processing/utils.py',
    'flow/plugins/__init__.py',
    'flow/plugins/parflow.py',
    'flow/__init__.py',
    'flow/api.py',
    'flow/configs',
    'flow/configs/__init__.py',
    'flow/configs/colorMaps.py',
]
...
MAIN_SCRIPT = 'pvw-flow.py'

# -----------------------------------------------------------------------------
# ParaView Flow TaskFlow
# -----------------------------------------------------------------------------

class ParaViewFlowTaskFlow(cumulus.taskflow.cluster.ClusterProvisioningTaskFlow):
    """
    {
    ...
    def start(self, *args, **kwargs):
        ...
        super(ParaViewFlowTaskFlow, self).start(self, *args, **kwargs)

    def terminate(self):
        super(ParaViewFlowTaskFlow, self).terminate()
        self.run_task(cleanup_proxy_entries.s())

    def delete(self):
        super(ParaViewFlowTaskFlow, self).delete()
        self.run_task(cleanup_proxy_entries.s())
...
def create_paraview_job(task, *args, **kwargs):
    ...
    for localFile in LOCAL_FILES:
        ...
        if not os.path.exists(srcFile):
            task.logger.error('Unable to locate file for upload. (%s)' % srcFile)
            return

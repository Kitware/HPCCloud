...
export default function taskflowsReducer(state = initialState, action) {
  switch (action.type) {
      ...
      if (action.taskflow.meta && action.taskflow.meta.jobs) {
        action.taskflow.meta.jobs.forEach((job) => {
          ...
          if (
            ...
            state.mapById[action.taskflow._id].jobMapById &&
            state.mapById[action.taskflow._id].jobMapById[job._id]
          ) {
            jobMapById[job._id].status =
              state.mapById[action.taskflow._id].jobMapById[job._id].status;
          }
        });
      }
    ...
    case Actions.UPDATE_TASKFLOW_JOB_LOG: {
      ...
      const jobMapById = Object.assign({}, taskflow.jobMapById, {
        [jobId]: job,
      });
      const newTaskflow = Object.assign({}, taskflow, { jobMapById });
      const mapById = Object.assign({}, state.mapById, {
        [taskflowId]: newTaskflow,
      });
    ...
    case Actions.UPDATE_TASKFLOW_TASK_LOG: {
      ...
      const taskMapById = Object.assign({}, taskflow.taskMapById, {
        [taskId]: task,
      });
    ...
    case Actions.UPDATE_TASKFLOW_JOB_STATUS: {
      ...
      const taskflow = state.mapById[taskflowId];
      if (
        taskflow.jobMapById[jobId] &&
        taskflow.jobMapById[jobId].status === status
      ) {
    ...
    case Actions.UPDATE_TASKFLOW_TASKS: {
      ...
      const task = Object.assign({}, state.mapById[taskflowId], {
        taskMapById,
      });
    ...
    case Actions.UPDATE_TASKFLOW_TASK_STATUS: {
      ...
      const taskflow = state.mapById[taskflowId];
      if (
        taskflow.taskMapById[taskId] &&
        taskflow.taskMapById[taskId].status === status
      ) {
    ...
    case Actions.BIND_SIMULATION_TO_TASKFLOW: {
      ...
      const taskflow = Object.assign({}, state.mapById[taskflowId], {

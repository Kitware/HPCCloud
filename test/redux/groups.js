...
describe('group actions', () => {
  ...
  const groups = [{ _id: 'abc', name: 'group1' }, { _id: 'def', name: 'group2' }];
  ...
  describe('simple actions', () => {
    ...
    it('updates local group', (done) => {
      ...
      const expectedAction = { type: Actions.SAVE_GROUP, index: 0, group: thisGroup };
    ...
    it('updates the active group in a list', (done) => {
      ...
      expect(Actions.updateActiveGroup(index))
        .toDispatchActions(expectedAction, done);

      expect(groupsReducer(deepClone(initialState), expectedAction).active)
        .toEqual(index);
    });
  });
  ...
  describe('async actions', () => {
    ...
    it('fetches groups', (done) => {
      const accessResponse = { _id: 'abc', name: 'group1', access: { users: groupUsers } };
    ...
    it('updates a remote group', (done) => {
      ...
      Actions.updateGroup(0, someGroup);
      Actions.updateGroup(0, someGroup);
      expect(Actions.updateGroup(0, someGroup))
        .toDispatchActions(expectedAction, done);
    ...
    it('removes users from a group', (done) => {
      ...
      expect(Actions.removeFromGroup('123', 'abc'))
        .toDispatchActions([], done);
      expect(client.removeUserFromGroup.calls.count()).toEqual(1);

      expect(Actions.removeFromGroup('123', ['abc', 'def', 'ghi']))
        .toDispatchActions([], done);
    ...
    it('deletes a group', (done) => {
      ...
      const expectedActions = { type: Actions.REMOVE_GROUP, group: someGroup, index: 0 };
      expect(Actions.deleteGroup(0, someGroup))
        .toDispatchActions(expectedActions, done);

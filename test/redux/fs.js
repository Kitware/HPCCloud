...
describe('fs', () => {
  ...
  describe('simple actions', () => {
    it('should update folder', (done) => {
      ...
      expect(Actions.updateFolder(folder)).toDispatchActions(
        expectedAction,
        complete(done)
      );

      expect(fsReducer(initialState, expectedAction)).toEqual(fsState);
    });

    // just reducer
    it('should open a folder', (done) => {
      const action = { type: Actions.TOGGLE_OPEN_FOLDER, folderId: folder._id };
      expect(Actions.toggleOpenFolder(folder._id, false)).toDispatchActions(
        [],
        complete(done)
      );
      ...
      newState.folderMapById[folder._id].open = !fsState.folderMapById[
        folder._id
      ].open;
      expect(fsReducer(fsState, action)).toEqual(newState);

      // closes
      expect(fsReducer(newState, action)).toEqual(fsState);
    });
  });
    ...
    it('should fetch folders and child items', (done) => {
      ...
      const expectedActions = [
        ...
        { type: Actions.CHILDREN_ITEMS, children: items, id: folder._id },
        {
          ...
          id: childFolders[0]._id,
        },
        {
          type: Actions.UPDATE_FOLDER,
          folder: childFolders[1],
          id: childFolders[1]._id,
        },
      ];
      ...
      expect(Actions.fetchFolder(folder._id, false)).toDispatchActions(
        expectedActions,
        complete(done)
      );
    });

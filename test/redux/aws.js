...
import awsReducer, { awsTemplate, initialState } from '../../src/redux/reducers/aws';
...
describe('aws', () => {
  ...
  describe('simple actions', () => {
    it('should add aws profile', (done) => {
      ...
      expect(awsReducer(awsState, expectedAction))
        .toEqual(expectedState);
    });

    it('should update active profile', (done) => {
      const expectedAction = { type: Actions.UPDATE_ACTIVE_AWS_PROFILE, index: 3 };
      expect(Actions.updateActiveProfile(3))
        .toDispatchActions(expectedAction, complete(done));
    ...
    it('should update profiles', (done) => {
      ...
      expect(awsReducer(initialState, expectedAction))
        .toEqual(expectedState);
    });
  });
  ...
  describe('async actions', () => {
    ...
    it('should fetch aws profiles', (done) => {
      ...
      expect(Actions.fetchAWSProfiles())
        .toDispatchActions(expectedAction, complete(done));
    });
    ...
    it('should remove aws profile', (done) => {
      ...
      setSpy(client, 'listAWSProfiles', profiles);
      expect(Actions.removeAWSProfile(0, profile))
        .toDispatchActions(expectedAction, complete(done));

      expectedAction = { type: Actions.REMOVE_AWS_PROFILE, index: 0 };
      expect(Actions.removeAWSProfile(0, { name: 'no id'}))
        .toDispatchActions(expectedAction, complete(done));
    });
    ...
    it('should update aws profile', (done) => {
      ...
      var expectedAction = { type: Actions.UPDATE_AWS_PROFILES, profiles: [ profile ] };
      expect(Actions.updateAWSProfile(0, profile, true))
        .toDispatchActions(expectedAction, complete(done));

      expectedAction = { type: Actions.SAVE_AWS_PROFILE, index: 0, profile: profile }
      expect(Actions.updateAWSProfile(0, profile, false))
        .toDispatchActions(expectedAction, complete(done));
    });
  });
});

...
describe('network', () => {
  describe('simple actions', () => {
    it('logs successful requests', (done) => {
      const expectedAction = { type: Actions.SUCCESS_NETWORK_CALL, id: '01', resp: {} };
      expect(Actions.successNetworkCall('01', {}))
      ...
      expect(netReducer(givenState, expectedAction))
        .toEqual(expectedState);
    });

    it('logs application network errors', (done) => {
      const someTimeout = setTimeout(() => {}, 50);
      const expectedAction = { type: Actions.ERROR_NETWORK_CALL,
        id: '01', resp: {}, errorTimeout: someTimeout + 1, errType: 'application' };
      expect(Actions.errorNetworkCall('01', {}))
        .toDispatchActions(expectedAction, complete(done));

      const givenState = deepClone(initialState);
      givenState.pending = { '01': { id: '01', resp: {} } };
      const expectedState = deepClone(givenState);
      expectedState.error = { '01': Object.assign({}, givenState.pending['01'], { invalid: false }) };
    ...
    it('logs form network errors', (done) => {
      const someTimeout = setTimeout(() => {}, 50);
      const expectedAction = { type: Actions.ERROR_NETWORK_CALL,
        id: '01', resp: {}, errorTimeout: someTimeout + 1, errType: 'form' };
      expect(Actions.errorNetworkCall('01', {}, 'form'))
        .toDispatchActions(expectedAction, complete(done));
    ...
    it('invalidates single network errors given an id', (done) => {
      const expectedAction = { type: Actions.INVALIDATE_ERROR,
        id: '01', errType: 'form' };
      expect(Actions.invalidateError('01', 'form'))
        .toDispatchActions(expectedAction, complete(done));

      const someTimeout = setTimeout(() => {}, 50);
      const givenState = deepClone(initialState);
      givenState.errorTimeout = someTimeout;
      givenState.error = { '01': { id: '01', resp: { data: { message: 'wow' } }, invalid: false } };
      givenState.activeErrors.form = ['01'];
      const expectedState = deepClone(givenState);
      expectedState.errorTimeout = null;
      expectedState.error = { '01': Object.assign({}, givenState.error['01'], { invalid: true }) };
    ...
    it('invalidates multiple network errors given ids', (done) => {
      const expectedAction = { type: Actions.INVALIDATE_ERRORS,
        ids: ['01', '02'], errType: 'form' };
      expect(Actions.invalidateErrors(['01', '02'], 'form'))
        .toDispatchActions(expectedAction, complete(done));

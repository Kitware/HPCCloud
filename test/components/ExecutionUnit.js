...
describe('ExecutionUnit', () => {
  ...
  it('render a log with several plain entries', () => {
    ...
    expect(logEntries[0].textContent).toMatch(/\[\d{2}:\d{2}:\d{2}\.\d{3}\] INFO: some message \[test\]/);
    expect(logEntries[1].textContent).toMatch(/\[\d{2}:\d{2}:\d{2}\.\d{3}\] WARN: some warning/);
    expect(logEntries[2].textContent).toMatch(new RegExp(JSON.stringify(myUnit.log[2].msg, null, 2)));
  });
  ...
  it('render a log with a complex entry (LogFold)', () => {
    ...
    expect(log).toExist();
    expect(log.props.header).toMatch(/\[\d{2}:\d{2}:\d{2}\.\d{3}\] INFO: some message/);
    expect(log.props.content).toEqual(JSON.stringify(myUnit.log[0].data, null, 2));
  });
  ...
  describe('updates', () => {
    ...
    it('updates when there\'s a new log', () => {
      myUnit.log.push({ msg: 'some warning', levelname: 'WARN', created: Date.now() + 20 });

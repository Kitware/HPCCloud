...
export default React.createClass({
    ...
    handleSubmit(event) {
        ...
        client.resetPassword(email)
            .then(resp => {
                this.setState({error: null, success:resp.data.message});
            })
            .catch(err => {
                this.setState({error: err.data.message, success: null});
            });
    },
    ...
    render() {
        ...
        <div className={layout.textCenter}>
            ...
            <form className={style.loginForm} onSubmit={this.handleSubmit}>
                ...
                <div>
                    <button className={style.loginButton}>Send <i className="fa fa-arrow-circle-o-right"></i></button>
                </div>
                {this.state.error && (

...
export default React.createClass({
    ...
    handleSubmit(event) {
        ...
        if (!this.state.error) {
            client.registerUser({login: this.refs.login.value, email: this.refs.email.value, password: this.state.password}, (error) => {
                if(error) {
                    this.setState({error});
                } else {
            ...
            <center>
                <form className={style.loginForm} onSubmit={this.handleSubmit}>
                    <label className={style.loginLabel}>
                        <input ref="login" type="login" placeholder="login" required/>
                    </label>
                    <label className={style.loginLabel}>

...
export default React.createClass({
    ...
    newSimulation(data, attachements) {
        const { name, description } = data,
            ...
            steps = Workflows[this.state.project.type].steps._initial_state,
            metadata = {},
            active = Workflows[this.state.project.type].steps._order[0],
            simulation = { name, description, steps, metadata, projectId, active };

        client.saveSimulation(simulation)
            .then(resp => this.context.router.replace('/View/Simulation/' + resp.data._id))
            .catch(err => {
                this.setState({ _error: err.data.message });

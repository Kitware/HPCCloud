...
export function registerUser({login, email, password}, cb) {
    const previousUser = cachedData.users[login];
    const display = email.split('@')[0].split('.').join(' ');
    if(previousUser) {
        cb('User ' + login + ' already exist');
    } else {
        cachedData.users[login] = { login, email, password, display };
        cb();
    }
}
    ...
    if(localStorage.token) {
        for(const l in cachedData.users) {
            if(cachedData.users[l].token === localStorage.token) {
                return cachedData.users[l].display;
            }
        }
    }
    return 'Unkown';

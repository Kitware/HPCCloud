...
export default React.createClass({
    ...
    runSimulation(event) {
        ...
        client.enableSimulationStep(this.props.simulation, NEXT_STEP)
            .then(ok => {
                client.activateSimualtionStep(this.props.simulation, NEXT_STEP)
                    .then(resp => {
                        client.disableSimulationStep(this.props.simulation, CURRENT_STEP)
                            .then(r => {
                                this.context.router.replace(['/View/Simulation', this.props.simulation._id, NEXT_STEP].join('/'));
                            })
                            .catch(err => {
                                console.log('Error: PyFrSym/run/disable', err);
                            });
                    })
                    .catch(err => {
            ...
            .catch(ko => {
                console.log('Can not enable Viz step');
            });
    },
